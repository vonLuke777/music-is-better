...
mkdir -p app/src/main/java/pl/speakersync/media

cat > app/src/main/java/pl/speakersync/media/MediaNotificationListener.kt <<'EOF'
...
echo "=== Naprawa AmplifierHero (Color.copy) ==="
HERO=app/src/main/java/pl/speakersync/ui/components/AmplifierHero.kt
if [ -f "$HERO" ]; then
  sed -i 's/Color\.White\.copy(0\.5f)/Color.White.copy(alpha = 0.5f)/g' "$HERO"
  sed -i 's/IndicatorGreen\.copy(0\.85f)/IndicatorGreen.copy(alpha = 0.85f)/g' "$HERO"
fi
